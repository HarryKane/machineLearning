...
class IPNN(object):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            # ---------- p --------------- embeddings层各fi向量的线性组合的2范数的平方
            self.p = None
            for i,split_wp in enumerate(tf.split(self.weights["WP"], num_or_size_splits=self.D1, axis=1)):
                if i== 0:
                    self.p = tf.reduce_sum(tf.square(tf.reduce_sum(tf.multiply(self.embeddings, split_wp),1)), 1,keep_dims=True)
                else:
                    self.p = tf.concat([self.p,tf.reduce_sum(tf.square(tf.reduce_sum(tf.multiply(self.embeddings, split_wp),1)), 1,keep_dims=True)],axis=1)
            # None * D1
            ...
            #train_op...
            self.loss = tf.losses.log_loss(self.y_, self.y)
            self.train_op = tf.train.GradientDescentOptimizer(learning_rate=self.learning_rate, use_locking=False).minimize(self.loss)
    ...
    def _initialize_weights(self):
        ...
        weights["WP"] = tf.Variable(
            tf.random_normal([self.field_size,self.D1], 0.0, 0.01),name="WP")
